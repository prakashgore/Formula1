...
with mlflow.start_run() as run:
    ...
    mlflow.sklearn.log_model(lr, "f1_race_winner_prediction_model", signature=signature)
    print("Model saved in run_id=%s" % run.info.run_id)
    
    # Register the model produced from your training job.
    print("test register_model.")
    mlflow.register_model(
        "runs:/{}/f1_race_winner_prediction_model".format(run.info.run_id), "f1_race_winner_prediction_model"
    )
    print("All done")
